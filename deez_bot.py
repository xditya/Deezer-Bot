...
from utils.utils_graphs import (
    create_graph_users,
    create_graph_top_downloaders,
    get_data_downloaders,
)

from telegram import MessageEntity, Update, InputMediaPhoto

from helpers.db_help import delete_banned, select_all_users, write_banned

from configs.customs import (
    not_found_query_gif,
    shazam_audio_query,
    shazam_function_msg,
    max_download_user_msg,
    help_msg,
    help_photo,
    feedback_text,
    what_can_I_do,
...
from configs.bot_settings import (
    download_dir_max_size,
    time_sleep,
    output_shazam,
    recorded_file_max_size,
    root_ids,
    bunker_channel,
    owl_channel,
    max_download_user,
...
from inlines.inline_keyboards import (
    create_keyboad_search,
    create_keyboard_settings,
    create_keyboard_qualities,
    create_shazamed_keyboard,
    create_keyboard_search_method,
    create_banned_keyboard,
    create_c_dws_user_keyboard,
...
def participant(user_id: int):
    if SetConfigs.__fsub_chat_id is None:
        return True
    try:
        bot.get_chat_member(SetConfigs.__fsub_chat_id, user_id)
    except:
        return False
...
def help_check_user(chat_id, date=None):
    if (mode_bot in strict_modes) and (not chat_id in root_ids):
        bot.send_message(chat_id=chat_id, text="BOT IS UNDER MAINTENANCE")
    ...
    if not user_exist:
        bot.send_message(
            chat_id=chat_id, text=startup_text, parse_mode=ParseMode.MARKDOWN
        )
...
def help_download(link, chat_id):
    ...
    if len(c_downloads) == max_download_user:
        bot.send_message(chat_id=chat_id, text=max_download_user_msg)
    ...
    t = magicThread(target=new_ins.download, args=(link,))

    c_downloads[hash_link] = {"link": link, "thread": t, "dw": new_ins}
...
def handle_callback_queries(update: Update, context):
    ...
    if mode == 0:
        bot.edit_message_text(
            chat_id=chat_id, message_id=msg_id, text=text, reply_markup=c_keyboard
        )

    bot.answer_callback_query(callback_query_id=query_id, text=answer)
...
def audio_handler(update: Update, context):
    ...
    try:
        ...
        out = c_file.download(custom_path=f"{output_shazam}{file_id}")
    ...
    if not resp:
        bot.send_message(chat_id=chat_id, text="I CANNOT SHAZAM THE SONG RETRY :(")
    ...
    bot.send_photo(
        ...
        caption=(
            shazam_audio_query % (artist, genre, album, label, release_date, title)
        ),
    )
...
def managing_banned_command(update: Update, context):
    ...
    bot.send_message(
        chat_id=chat_id, text="Banned users", reply_markup=create_banned_keyboard()
    )
...
def add_banned_command(update: Update, context):
    ...
    bot.send_message(chat_id=chat_id, text="Send me the user to ban")
...
def shazam_command(update: Update, context):
    ...
    bot.send_message(chat_id=chat_id, text=shazam_function_msg)
...
def kill_dw_command(update: Update, context):
    ...
    if not c_downloads:
        bot.send_message(chat_id=chat_id, text="There aren't any download in progress")

        return

    keybords_dws = create_c_dws_user_keyboard(c_downloads)

    for c_download, keyboard in zip(c_downloads.values(), keybords_dws):
        c_text = f"{c_download['link']} in {c_user_data['quality']}"

        bot.send_message(chat_id=chat_id, text=c_text, reply_markup=keyboard)
...
def reasons_command(update: Update, context):
    ...
    bot.send_message(chat_id=chat_id, text=reasons_text)
...
def help_command(update: Update, context):
    ...
    bot.send_photo(chat_id=chat_id, photo=help_photo, caption=help_msg)

    bot.send_message(chat_id=chat_id, text=what_can_I_do)
...
def feedback_command(update: Update, context):
    ...
    bot.send_message(chat_id=chat_id, text=feedback_text)
...
def donate_command(update: Update, context):
    ...
    bot.send_message(
        chat_id=chat_id, text=donate_text, reply_markup=create_info_keyboard()
    )
...
def graphs_command(update: Update, context):
    ...
    for user, num_downloads in zip(users, nums_downloads):
        caption_text_download_graph += (
            f"{times} Place: User `{user}` with {num_downloads} downloads\n"
        )
        times += 1
    ...
    medias = [
        ...
        InputMediaPhoto(media=users_graph),
    ]
...
def msgs_handler(update: Update, context):
    ...
    if chat_id in roots_data:
        if roots_data[chat_id]["stage"] == "add_banned":
            ...
            bot.send_message(chat_id=chat_id, text=f"User {text} has been blocked")
    ...
    bot.send_message(
        chat_id=chat_id, text=text, reply_markup=create_keyboad_search(text)
    )
...
start_handler = CommandHandler("start", start_command, filters=~to_ban, run_async=True)

dispatcher.add_handler(start_handler)

settings_handler = CommandHandler(
    "settings", settings_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(settings_handler)

quality_handler = CommandHandler(
    "quality", quality_command, filters=~to_ban, run_async=True
)
...
managing_banned_handler = CommandHandler(
    "managing_banned", managing_banned_command, filters=to_root, run_async=True
)

dispatcher.add_handler(managing_banned_handler)

add_banned_handler = CommandHandler(
    "add_banned", add_banned_command, filters=to_root, run_async=True
)

dispatcher.add_handler(add_banned_handler)

shazam_handler = CommandHandler(
    "shazam", shazam_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(shazam_handler)

kill_dw_handler = CommandHandler(
    "kill_dw", kill_dw_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(kill_dw_handler)

info_handler = CommandHandler("info", info_command, filters=~to_ban, run_async=True)

dispatcher.add_handler(info_handler)

reasons_handler = CommandHandler(
    "reasons", reasons_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(reasons_handler)

help_handler = CommandHandler("help", help_command, filters=~to_ban, run_async=True)

dispatcher.add_handler(help_handler)

feedback_handler = CommandHandler(
    "feedback", feedback_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(feedback_handler)

donate_handler = CommandHandler(
    "donate", donate_command, filters=~to_ban, run_async=True
)

dispatcher.add_handler(donate_handler)

graphs_handler = CommandHandler(
    "graphs", graphs_command, filters=~to_ban, run_async=True
)
...
send_global_msg_handler = MessageHandler(
    filter_owl_channel, send_global_msg_command, run_async=True
)
...
control_links = MessageHandler((filter_url & ~to_ban), controls_links, run_async=True)
...
inline_queries = InlineQueryHandler(handle_inline_queries, run_async=True)

dispatcher.add_handler(inline_queries)

callback_queries = CallbackQueryHandler(handle_callback_queries, run_async=True)
